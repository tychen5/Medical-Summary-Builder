...
2.  **Vector Indexing**: Each text chunk is converted into a vector embedding using a powerful model (e.g., `Qwen/Qwen3-Embedding-8B` or `text-embedding-3-small`) and stored in a Pinecone index (need to preset it via its console first and named as medical-summary-index). This creates a searchable knowledge base from the medical record. This step can be skipped on subsequent runs if the index is already populated.
